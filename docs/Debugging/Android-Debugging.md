...
​       Run your app on a connected device or emulator. If using a connected device, be sure you have            	   [enabled USB debugging](https://developer.android.com/studio/run/device.html?hl=ko#setting-up).
...
1. 연결된 장치 또는 에뮬레이터에서 앱을 실행합니다. 연결된 장치를 사용하는 경우 [`USB 디버깅을 활성화`](https://developer.android.com/studio/run/device.html?hl=ko#setting-up)했는지 확인합니다.
   ...
   In Android Studio, select **View > Tool Windows > Logcat** to open [Logcat](https://developer.android.com/studio/debug/am-logcat.html?hl=ko).
...
3. 창의 맨 위에 있는 드롭-다운에서 장치 및 프로세스를 선택합니다.
   ...
   Click **Screen Capture**![img](./Images/monitor-screenshot.png)The screenshot appears in a **Screenshot Editor** window.
...
5. Logcat 창의 왼쪽에서 **Screen Record**![img](./Images/am-ivideo.png)를 클릭합니다.
